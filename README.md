## 新增 select  
```
app.mysql.select(table_name,{
  alias:"a",
  join:[
    {table:"abc",alias:"b",on:["a.id","b.id"]}
    {table:"abcd",alias:"c",on:["a.id","c.id"]}
  ],
  where:{
     name:{
      operator:"<>",
     ...
     intro:{
      operator:"like",
      value:app.mysql.escape('%'+keywords+'%');
     }
  },
});
...
```
## 新增literals 
> exports.last_insert_id = new Literal('LAST_INSERT_ID()');
