...
proto.count = function* (table, where,options={}) {
  var column=options.column?options.column:"*";
  var sql = this.format('SELECT COUNT('+column+') as count FROM ??', [ table ]);
      if(options.alias){
        sql+=` ${options.alias} `;
      }
...
proto.select = function* (table, options) {
  ...
  var sql = this._selectColumns(table, options.columns);
    if(options.alias){
      sql+=this._alias(options.alias);
    }
    if(options.join && options.join.length){
      sql+=this._join(options.join);
    }
    sql+=this._where(options.where);
    sql+=this._orders(options.orders);
    sql+=this._limit(options.limit, options.offset);
  if(options.log){
    console.log(sql);
  }
  debug('select(%j, %j) \n=> %j', table, options, sql);
...
proto._where = function(where) {
  if(typeof(where)=='string' && where.trim().length){
    return " WHERE "+where;
  }
...
proto._selectColumns = function(table, columns) {
  ...
  if (columns === '*') {
    ...
    sql = this.format('SELECT '+columns.join(",")+' FROM ??', [ table ]);
  }
  return sql;
};
...
proto._orders = function(orders) {
  ...
  const values = [];
  // if(util.isObject(orders)){
  //   Object.keys(orders).map((key)=>{
  //     values.push(this.escapeId(key) +" "+orders[key]);
  //   })
  // }else{
